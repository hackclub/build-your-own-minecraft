...
export default class Block {
  ...
  // lighting = 6 * 6 (6 vertices, 6 sides, 1 lighting point per vertex -> can interpolate)
  // rgb = 3 * 6 * 6 (6 vertices, 6 sides, 3 color channels per vertex -> can interpolate, but we don't)
  ...
  get colors() {
    ...
    switch (this.type) {
      case BLOCK_TYPES.LEAF.id:
        return repeatArray(normalizeRgb(72, 181, 24), 3 * 6 * 6)
      case BLOCK_TYPES.SAND.id:
        if (window.gameDebug)
          return repeatArray(normalizeRgb(203, 189, 147), 3 * 6 * 6)
      ...
      case BLOCK_TYPES.DIRT_GRASS.id:
        if (window.gameDebug) return repeatArray([0.0, 1.0, 0.0], 3 * 6 * 6)
      default:
  ...
  get textureDepth() {
    switch (this.type) {
      case BLOCK_TYPES.DIRT_GRASS.id: {
        // Grass on top, dirt grass combo on sides
        return [
          ...repeatArray([this.type], 6 * 2),
          ...new Array(6).fill(BLOCK_TYPES.GRASS.id),
          ...new Array(6).fill(BLOCK_TYPES.DIRT.id),
          ...repeatArray([this.type], 6 * 2)
        ]
      }
      case BLOCK_TYPES.OAK.id: {
        return [
          ...repeatArray([this.type], 6 * 2),
          ...new Array(6).fill(BLOCK_TYPES.INNER_LOG.id),
          ...repeatArray([this.type], 6 * 3)
        ]
      }
      default:
        return new Array(6 * 6).fill(this.type)
    }
  }
